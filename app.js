...
const icons = {
    ...
    clock2: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
...
function loadArticleReactions(articleId) {
    reactionsRef.child(articleId).on('value', (snapshot) => {
        const data = snapshot.val() || {};
...
async function toggleReaction(articleId, reactionType) {
    const userKey = `${articleId}_${reactionType}`;
    const hasReacted = state.userReactions[userKey];
    
    try {
        if (hasReacted) {
            // Remove reaction
            await reactionsRef.child(`${articleId}/${reactionType}`).transaction((current) => {
                return Math.max(0, (current || 0) - 1);
            });
            delete state.userReactions[userKey];
        } else {
            // Add reaction
            await reactionsRef.child(`${articleId}/${reactionType}`).transaction((current) => {
                return (current || 0) + 1;
            });
            state.userReactions[userKey] = true;
...
function animateReaction(reactionType) {
...
function getTotalReactions(articleId) {
    const reactions = state.articleReactions[articleId] || {};
    return (reactions.clap || 0) + (reactions.fire || 0) + (reactions.idea || 0) + (reactions.heart || 0);
}
...
function trackReadingHistory(article) {
    ...
    state.readingHistory = state.readingHistory.filter(h => h.id !== article.id);
    ...
    state.readingHistory.unshift({
        ...
        lastRead: Date.now(),
        progress: state.readingProgress[article.id] || 0
    });
    
    // Keep only last 20
    state.readingHistory = state.readingHistory.slice(0, 20);
    localStorage.setItem('readingHistory', JSON.stringify(state.readingHistory));
}

function updateReadingProgress(articleId, progress) {
    state.readingProgress[articleId] = progress;
    ...
    // Update in history
    const historyItem = state.readingHistory.find(h => h.id === articleId);
    if (historyItem) {
        historyItem.progress = progress;
        historyItem.lastRead = Date.now();
        localStorage.setItem('readingHistory', JSON.stringify(state.readingHistory));
    }
}
...
function setupScrollTracking(articleId) {
    ...
    window.currentScrollHandler = handleScroll;
}
...
function getReadingHistory() {
    return state.readingHistory.filter(h => {
        // Check if article still exists
        return articles.some(a => a.id === h.id);
    }).slice(0, 5);
}
...
function continueReading(articleId) {
    const progress = state.readingProgress[articleId] || 0;
    ...
    setTimeout(() => {
        ...
        if (article && progress > 0) {
            ...
            const scrollTo = articleTop + (articleHeight * progress / 100) - window.innerHeight * 0.3;
...
function handleRoute() {
    ...
    // Cleanup scroll tracking from previous article
    ...
    if (hash.startsWith('#/admin/' + ADMIN_SECRET_PATH)) {
        ...
        if (hash === '#/articles') {
            ...
            if (state.selectedArticle) {
                trackVisit(id);
                // Load likes, reactions, and comments for this article
                loadArticleLikes(id);
                loadArticleReactions(id);
                loadArticleComments(id);
                // Track reading history
                trackReadingHistory(state.selectedArticle);
                // Setup scroll tracking after render
                setTimeout(() => setupScrollTracking(id), 100);
            }
        } else {
...
function renderHomePage() {
    ...
    if (readingHistory.length > 0) {
        html += '<section class="continue-reading-section"><div class="section-header"><h2 class="section-title">' + icons.clock2 + ' Continue Reading</h2><span class="section-link" onclick="clearReadingHistory()">Clear history</span></div><div class="continue-reading-grid">';
        readingHistory.forEach(function(item) {
            const progress = item.progress || 0;
            const timeAgo = formatTimeAgo(item.lastRead);
            html += '<div class="continue-card" onclick="continueReading(\'' + item.id + '\')"><div class="continue-image-wrapper"><img src="' + (item.image || 'https://images.unsplash.com/photo-1457369804613-52c61a468e7d?w=800') + '" alt="' + item.title + '" class="continue-image"><div class="continue-progress"><div class="continue-progress-bar" style="width: ' + progress + '%"></div></div></div><div class="continue-content"><span class="continue-category">' + item.category + '</span><h4 class="continue-title">' + item.title + '</h4><div class="continue-meta"><span>' + progress + '% complete</span><span>·</span><span>' + timeAgo + '</span></div></div><div class="continue-arrow">' + icons.arrowRight + '</div></div>';
        });
        html += '</div></section>';
    }
...
function renderArticleDetail() {
    ...
    // Reactions
    const reactions = state.articleReactions[article.id] || { clap: 0, fire: 0, idea: 0, heart: 0 };
    const totalReactions = getTotalReactions(article.id);
    
    // Build reactions HTML
    let reactionsHtml = '<div class="reactions-bar">';
    REACTION_TYPES.forEach(function(r) {
        const userKey = article.id + '_' + r.id;
        const hasReacted = state.userReactions[userKey];
        const count = reactions[r.id] || 0;
        reactionsHtml += '<button class="reaction-btn ' + (hasReacted ? 'active' : '') + '" onclick="toggleReaction(\'' + article.id + '\', \'' + r.id + '\')" title="' + r.label + '"><span class="reaction-emoji">' + r.emoji + '</span><span class="reaction-count">' + count + '</span></button>';
    });
    reactionsHtml += '</div>';
    ...
    let floatingBarHtml = '<div class="floating-action-bar"><div class="floating-reactions">';
    REACTION_TYPES.forEach(function(r) {
        const userKey = article.id + '_' + r.id;
        const hasReacted = state.userReactions[userKey];
        const count = reactions[r.id] || 0;
        floatingBarHtml += '<button class="floating-reaction-btn ' + (hasReacted ? 'active' : '') + '" onclick="toggleReaction(\'' + article.id + '\', \'' + r.id + '\')" title="' + r.label + '"><span class="reaction-emoji">' + r.emoji + '</span>' + (count > 0 ? '<span class="floating-count">' + count + '</span>' : '') + '</button>';
    });
    floatingBarHtml += '</div><div class="floating-divider"></div><div class="floating-share">';
    floatingBarHtml += '<button class="floating-share-btn" onclick="shareOnTwitter(state.selectedArticle)" title="Share on X">' + icons.twitter + '</button>';
    floatingBarHtml += '<button class="floating-share-btn" onclick="shareOnFacebook(state.selectedArticle)" title="Share on Facebook">' + icons.facebook + '</button>';
    floatingBarHtml += '<button class="floating-share-btn" onclick="shareOnWhatsApp(state.selectedArticle)" title="Share on WhatsApp">' + icons.whatsapp + '</button>';
    floatingBarHtml += '<button class="floating-share-btn" onclick="copyArticleLink()" title="Copy link">' + icons.copyLink + '</button>';
    floatingBarHtml += '</div><div class="floating-total">' + totalReactions + ' reactions</div></div>';
    
    // Reading progress bar
    const progressHtml = '<div class="reading-progress-container"><div class="reading-progress-bar" style="width: ' + (state.readingProgress[article.id] || 0) + '%"></div></div>';
    
    return progressHtml + floatingBarHtml + '<article class="article-detail"><span class="back-link" onclick="navigate(\'#/articles\')">← Back to articles</span><header class="article-header"><div class="article-meta" style="margin-bottom: 1rem;"><span class="category-tag">' + article.category + '</span><span>' + (article.readTime || '5 min') + ' read</span></div><h1 class="article-detail-title">' + article.title + '</h1><div class="article-detail-meta"><span>By <span class="author-name">' + (article.author || 'Unknown') + '</span></span><span>' + formatDate(article.date) + '</span></div></header>' + (article.image ? '<img src="' + article.image + '" alt="' + article.title + '" class="article-detail-image">' : '') + '<div class="article-body">' + article.content + '</div><div class="article-engagement-inline">' + reactionsHtml + '</div><div class="article-tags"><div class="article-tags-title">Tagged</div><div class="tags-list">' + tagsHtml + '</div></div>' + commentsHtml + '</article>';
}
